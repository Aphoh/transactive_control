...
class Office:
    def __init__(
        ...
        self._start_timestamp = pd.Timestamp(
            year=2012, month=1, day=2, hour=0, minute=0
        )
        self._end_timestamp = pd.Timestamp(
            year=2012, month=12, day=30, hour=0, minute=0
        )
        self._end_timestamp = pd.Timestamp(
            year=2012, month=12, day=30, hour=0, minute=0
        )
    ...
    def _create_agents(self):
        ...
        players_dict["player_0"] = DeterministicFunctionPerson(
            my_baseline_energy, points_multiplier=100
        )
        players_dict["player_1"] = DeterministicFunctionPerson(
            my_baseline_energy, points_multiplier=100
        )
        players_dict["player_2"] = DeterministicFunctionPerson(
            my_baseline_energy, points_multiplier=100
        )
        players_dict["player_3"] = DeterministicFunctionPerson(
            my_baseline_energy, points_multiplier=100
        )
        players_dict["player_4"] = DeterministicFunctionPerson(
            my_baseline_energy, points_multiplier=100
        )
        players_dict["player_5"] = DeterministicFunctionPerson(
            my_baseline_energy, points_multiplier=100
        )
        players_dict["player_6"] = DeterministicFunctionPerson(
            my_baseline_energy, points_multiplier=100
        )
        players_dict["player_7"] = DeterministicFunctionPerson(
            my_baseline_energy, points_multiplier=100
        )
    ...
    def step(self, prices):
        ...
        for player_name in self.players_dict:
            ...
            player_reward = Reward(
                player_energy, prices, player_min_demand, player_max_demand
            )
    ...
    def price_signal(self, day=45):
        ...
        def optimise_24h(netdemand_24, price_24):
            ...
            def objective(x):
                ...
                lambd = 0.005
                return sum(np.maximum(cost, 0)) + lambd * sum(
                    abs(x - 0.1 * netdemand_24)
                )
...
def main():
    ...
    with open("temp_reward_values.txt", "w") as f:
        while not end:
            ...
            points, last_demand, last_player_ideal, reward, end = test_office.step(
                prices
            )
