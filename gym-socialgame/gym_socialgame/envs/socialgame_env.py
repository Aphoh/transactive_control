...
class SocialGameEnv(gym.Env):
    ...
    def check_valid_init_inputs(
        ...
        assert isinstance(
            one_price, int
        ), "Variable one_price is not of type Int. Instead got type {}".format(
            type(one_price)
        )
        assert (
            366 > one_price and one_price > -2
        ), "Variable one_price out of range [-1,365]. Got one_price = {}".format(
            one_price
        )
        ...
        ), "Variable one_price is not of type Boolean. Instead got type {}".format(
        ...
        assert isinstance(
            pricing_type, str
        ), "Variable pricing_type is not of type String. Instead got type {}".format(
            type(pricing_type)
        )
        assert pricing_type.upper() in [
            "RTP",
            "TOU",
