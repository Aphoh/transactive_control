...
def eval_policy_helper(
    args, model, num_eval_episodes, response=None, list_reward_per_episode=False
):
    """
    ...
    for episode_i in range(num_eval_episodes):
        ...
        curr_reward, _ = evaluate_policy(
            model, env, 1, return_episode_rewards=list_reward_per_episode
        )
...
def eval_policy(model, args, num_eval_episodes: int, list_reward_per_episode=False):
    ...
    if args.random:
        ...
        print(
            "Standard Deviation  of Linear Reward: {:.3f}".format(np.std(eval_rewards))
        )
        print("*****" * 30)
        ...
        print(
            "Standard Deviation of Thresh. Reward: {:.3f}".format(np.std(eval_rewards))
        )
        print("*****" * 30)
...
def get_environment(args, eval=False, response=None):
    ...
    if eval:
        ...
        if args.random:
            socialgame_env = gym.make(
                "gym_socialgame:socialgame_dr-v0",
                action_space_string=action_space_string,
                response_type_string=args.response,
                one_price=args.one_price,
                number_of_participants=args.num_players,
...
def parse_args():
    ...
    parser.add_argument(
        "--one_price",
        help="Specific Day of the year to Train on (default = 0, train over entire yr)",
        type=int,
        default=0,
    ...
    parser.add_argument(
        "--random",
        help="Whether to use domain randomization (DR), default = F",
        type=str,
        default="F",
        choices=["T", "F"],
    )
    parser.add_argument(
        "--low",
        help="Lower bound for uniform noise to response function, default = 0",
        type=int,
        default=0,
    )
    parser.add_argument(
        "--high",
        help="Upper bound for uniform noise to response function, default = 50",
        type=int,
        default=50,
    )
    parser.add_argument(
        "--distr",
        help="Distribution for noise. Currently only Uniform distr. supported.",
        type=str,
        default="U",
        choices=["U", "G"],
    )
